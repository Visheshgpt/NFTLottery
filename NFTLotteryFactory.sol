...
contract LotteryFactory is Ownable {
    event NewLotteryContract(address indexed lottery);

    constructor() public {
    }

    function deployLottery(address _admin) external onlyOwner {


        bytes memory bytecode = type(NFTLottery).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(block.timestamp));
        address lotteryAddress;

        assembly {
            lotteryAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        NFTLottery(lotteryAddress).transferOwnership(
            _admin
        );

        emit NewLotteryContract(lotteryAddress);
    }
}
