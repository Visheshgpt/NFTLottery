...
contract NFTLottery is  IERC721Receiver, Ownable {
    ...
    function PickWinner(uint _lotteryid) external {
       
       Lottery storage lot = lotteryDetails[_lotteryid];
       
       require(lot.resultTime <= block.timestamp, "WINNING TIME NOT REACHED" );
       require (!(lot.staking), "Staking live");
       require (!(lot.winnerAnnounced), "Winner Already Announced");
       uint256 pickWinner = random(random2(msg.sender),lot.stakers.length) + 1;
       lot.winnerAddress = lot.stakers[pickWinner] ; 
       lot.winnerAnnounced = true;
        
       nftredeem.safeTransferFrom(address(this), lot.winnerAddress, lot.nftId);    
       emit winner(_lotteryid, lot.winnerAddress, lot.nftId);
    }
    ...
    function startStaking(uint _lotteryId, uint256 _resultTime) external onlyOwner {
        
        Lottery storage lot = lotteryDetails[_lotteryId];
       
        lot.staking = true;
        lot.resultTime = _resultTime;
    }
    
    function stopStaking(uint _lotteryId) external onlyOwner {
    ...
    function withdrawAdmin(IERC721 _nftaddress, uint256 _id, address admin) external onlyOwner {
        _nftaddress.safeTransferFrom(address(this), admin, _id); 
    }
    
    function withdrawtokens(IERC20 tokenaddress, address _admin, uint256 _amount) external onlyOwner {
        tokenaddress.transfer(_admin , _amount);  
    }
    
    function createnewLottery(uint _nftId) external onlyOwner {
